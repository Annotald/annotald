...
function publish(data, opts, tutorials) {
    ...
    var out = '',
        ...
        noview = new template.Template(env.dirname + '/' + templatePath + '/tmpl');
    ...
    function find(spec) {
        return data().get( data().filter(spec) );
    }
    ...
    data().order(['longname', 'version', 'since']);

    // kinds of containers
    var globals = find( {kind: ['member', 'function', 'constant', 'typedef'],
    ...
    data().each(function (doclet) {
        if (!doclet || !doclet.meta || !doclet.meta.path) {
            return;
        }
        var filename = doclet.meta.path + "/" + doclet.meta.filename;
        if (sourceFiles.indexOf(filename) == -1) {
            sourceFiles.push(filename);
        }
    });

    fs.mkPath(outdir + "/src");
    ...
    sourceFiles.forEach(function (file) {
        ...
        src = src.map(function(v, i) {
            return "<a name='L" + (i+1) + "'></a>" + v;
        });
        var data = {
            filename: file,
            src: src.join("\n")
        };
        var htmlsrc = noview.render("srcpage.tmpl", data);
        fs.writeFileSync(outdir + "/src/" + file.split("/").pop() + ".html",
                         htmlsrc);
    });
