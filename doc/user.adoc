...
// TODO: centering pictures sometimes doesn’t work
...
// TODO: this paragraph isn’t really applicable yet.
...
`-q` [[cmdline-q]]:: Deactivate the time-logging function.
// TODO: link to doc
...
`logDetail` [[logdetail]]:: This variable shoudl be a Boolean value (`true` or
...
writing their own javascript functions
// TODO: link
...
`rewriteIndices`:: By default, Annotald normalizes the indices of files
...
// TODO: we could instead record the validation as a single undo step.
// Drawback: undoing past the validate will erase the validation also.
// Which is worse?
...
this feature is disabled in the `settings.py` file.
...
- the tree itself
- an optional ID node
// TODO: link to defintion
// TODO: metadata

Uniquely, the outermost set of parentheses has no label.

The ID must be a leaf node, with label =ID= and text as described below.
// TODO
...
Often, the label is a word’s POS tag.  The text can be arbitrary text,
except it must not contain =(= or =)= characters.
